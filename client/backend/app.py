...
from agents.items import ToolCallOutputItem, ToolCallItem
...
async def connect_endpoint(request: Dict[str, Any]):
    ...
    try:
        ...
        metis_mcp_server = MCPServerStdio(
            ...
            params={
                "command": "npx",
                "args": ["-y", "mcp-remote", "http://localhost:9999/mcp"],
            },
...
async def send_message(session_id: str, request: Dict[str, Any]):
    ...
    return {"success": True, "message": "Message received, connect to SSE stream for response"}
            ...
            async for event in result.stream_events():
                ...
                if event.type == "raw_response_event":
                    ...
                    if isinstance(data, ResponseTextDeltaEvent):
                        ...
                        if hasattr(data.item, 'name') and hasattr(data.item, 'type') and getattr(data.item, 'type') == 'function_call':
                            ...
                            call_id = data.item.raw_item.get('call_id') if hasattr(data.item, 'raw_item') else None
                    ...
                    elif isinstance(data, ResponseOutputItemAddedEvent) and isinstance(data.item, ResponseFunctionToolCall):
                    ...
                    elif isinstance(data, ResponseOutputItemDoneEvent) and isinstance(data.item, ResponseFunctionToolCall):
                    ...
                    if isinstance(item, ToolCallOutputItem):
                        ...
                        call_id = item.raw_item.get('call_id') if hasattr(item, 'raw_item') else None
                        tool_name = tool_call_names.get(call_id)
                        
                        print(f"🔧 BACKEND: Tool response - call_id: {call_id}, tool_name: {tool_name}")
